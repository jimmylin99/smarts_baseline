...
from smarts.core.agent_interface import OGM, RGB, NeighborhoodVehicles
from smarts.core.controllers import ActionSpaceType

MAX_LANES = 5  # The maximum number of lanes we expect to see in any scenario.
lane_crash_ttc_flag = False  # used for training to signal a flipped car
lane_crash_mindist_flag = False
intersection_crash_flag = False  # used for training to signal intersect crash
lane_ttc4reward = 1
lane_mindist4reward = 100

CRASH_TTC_THRESHOLD = 0.1  # original 0.1
CRASH_MINDIST_THRESHOLD = 6  # original 6

# ==================================================
# Actuator Action Space
# throttle, brake, steering rate
# ==================================================
...
def ttc_by_path(ego, wp_paths, neighborhood_vehicle_states, ego_closest_wp):
    global lane_crash_ttc_flag
    global lane_crash_mindist_flag
    global lane_ttc4reward
    global lane_mindist4reward
    global intersection_crash_flag

    # init flag, dist, ttc, headings
    lane_crash_ttc_flag = False
    lane_crash_mindist_flag = False
    lane_ttc4reward = 1
    lane_mindist4reward = 100
    ...
    if not close_nv_index.size:
        ...
        for i, lane_index in enumerate(lane_index_list):
            ...
            if ego_closest_wp.lane_index in ep_closed_wp_lane_index:
                if min_distance < CRASH_MINDIST_THRESHOLD:
                    lane_mindist4reward = min_distance
                    lane_crash_mindist_flag = True
            ...
            for j in ep_closed_wp_lane_index:
                ...
                if j == ego_closest_wp.lane_index:
                    if ttc < CRASH_TTC_THRESHOLD:
                        lane_ttc4reward = ttc
                        lane_crash_ttc_flag = True
...
def observation_adapter(env_obs):
    ...
    # print('waypoint path ID: ', max_len_lane_index)
...
def reward_adapter(env_obs, env_reward):
    ...
    speed_reward = env_obs.ego_vehicle_state.speed
    ...
    if lane_crash_ttc_flag:
        # crash_penalty = -5
        delta = CRASH_TTC_THRESHOLD - lane_ttc4reward
        norm_delta = delta / CRASH_TTC_THRESHOLD
        crash_ttc_penalty = 1 - math.exp(norm_delta)  # non-positive number
    else:
        crash_ttc_penalty = 0

    if lane_crash_mindist_flag:
        delta = CRASH_MINDIST_THRESHOLD - lane_mindist4reward
        norm_delta = delta / CRASH_MINDIST_THRESHOLD
        crash_mindist_penalty = 1 - math.exp(norm_delta)  # non-positive
    else:
        crash_mindist_penalty = 0

    # penalise close proximity to intersection cars
    if intersection_crash_flag:
        crash_intersection_penalty = -5
    else:
        crash_intersection_penalty = 0

    total_reward = np.sum([reached_goal,
                           5 * speed_reward])
    total_penalty = np.sum([serious_fault,
                            1 * center_penalty])
                            # 1 * crash_ttc_penalty,
                            # 1 * crash_mindist_penalty,
                            # 1 * crash_intersection_penalty])

    unnorm_ret = (total_reward + total_penalty)

    # print('reward {:.2f} speed {:.2f} tot_reward {:.2f} tot_pen {:.2f} \n'
    #       'ttc_pen {:.2f} mindist_pen {:.2f} inter_pen {:.2f}'.format(
    #         unnorm_ret, env_obs.ego_vehicle_state.speed, total_reward, total_penalty,
    #         crash_ttc_penalty, crash_mindist_penalty, crash_intersection_penalty))

    # assert crash_ttc_penalty == 0
    # assert crash_mindist_penalty == 0
    # assert crash_intersection_penalty == 0

    # time.sleep(1)

    ret = unnorm_ret / 200.0
